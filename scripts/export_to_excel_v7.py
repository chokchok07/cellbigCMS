...
admin_data = [
    ...
    ["Admin/Auth", "POST", "/auth/register", "운영자/관리자 회원가입\n(가입 후 PENDING 상태)", "Body:\n- username\n- password\n- email\n- fullName", "{\n  \"success\": true,\n  \"data\": {\n    \"userId\": \"user_123\",\n    \"status\": \"PENDING_APPROVAL\",\n    \"message\": \"관리자 승인 대기\"\n  }\n}"],
    ["Admin/Auth", "GET", "/auth/me", "내 정보 및 권한 조회", "-", "{\n  \"success\": true,\n  \"data\": {\n    \"userId\": \"u1\",\n    \"username\": \"admin\",\n    \"role\": \"super_admin\",\n    \"scopeIds\": [],\n    \"permissions\": [\"device:read\"]\n  }\n}"],
    ["Admin/Auth", "PUT", "/auth/me", "내 정보 수정\n(본인 정보만 수정 가능)", "Body:\n- currentPassword\n- newPassword\n- fullName", "{\n  \"success\": true,\n  \"data\": {}\n}"],
    
    ["Admin/Users", "GET", "/users", "사용자 목록 조회\n(필요 권한: super_admin)", "-", "{\n  \"success\": true,\n  \"data\": [\n    { \"userId\": \"u1\", \"username\": \"m1\", \"role\": \"local_manager\", ... }\n  ]\n}"],
    ["Admin/Users", "POST", "/users", "새 운영자 계정 생성\n(필요 권한: super_admin 등)", "Body:\n- username\n- password\n- fullName\n- role\n- scopeIds", "{\n  \"success\": true,\n  \"data\": { \"userId\": \"u2\" }\n}"],
    ["Admin/Users", "PUT", "/users/{userId}", "운영자 정보 및 권한 수정\n(필요 권한: super_admin 등)\n* 여기서 하위 관리자의 scopeIds 할당", "Body:\n- role\n- status\n- scopeIds (예: [\"store_busan\"])", "{\n  \"success\": true,\n  \"data\": {\n    \"userId\": \"u2\",\n    \"role\": \"local_manager\",\n    \"scopeIds\": [\"store_busan\"]\n  }\n}"],
    
    ["Admin/Products", "GET", "/products", "제품 목록 조회", "-", "{\n  \"success\": true,\n  \"data\": [ { \"productId\": \"p1\", \"name\": \"...\" } ]\n}"],
    ["Admin/Products", "POST", "/products", "제품 생성\n(필요 권한: super_admin)", "Body:\n- name\n- description (제품 메타데이터)", "{\n  \"success\": true,\n  \"data\": { \"productId\": \"p2\" }\n}"],
    ...
    ["Admin/Devices", "PUT", "/admin/devices/{deviceId}/approve", "디바이스 승인 및 스토어 할당\n(필요 권한: super_admin, local_manager)", "Body:\n- storeId\n- packageId\n- status", "{\n  \"success\": true,\n  \"data\": {}\n}"],
    
    ["Admin/License", "POST", "/licenses", "라이선스(시리얼키) 발급\n(필요 권한: super_admin)", "Body:\n- productId\n- type\n- durationDays\n- count", "{\n  \"success\": true,\n  \"data\": [\n    \"License-ABCD-1234\",\n    \"License-EFGH-5678\"\n  ]\n}"]
]
...
def create_sheet(title, data):
    ...
    col_widths = {'A': 15, 'B': 10, 'C': 45, 'D': 40, 'E': 40, 'F': 50}
...
wb.save(filepath)
print("Updated Excel file with explicit user-authority update logic!")
