...
editor_html = re.sub(r'<!-- Right Column: Stores -->.*?</div>\s*</div>', r'</div>', editor_html, flags=re.DOTALL)
...
editor_html = re.sub(r'<!-- Metadata Section -->.*?</div>\s*</div>', r'<!-- Metadata removed for editor -->', editor_html, flags=re.DOTALL)

# Remove JS dynamic injection code block completely
editor_html = re.sub(r'// Handle New Mode.*?</script>', r'</script>', editor_html, flags=re.DOTALL)
# Remove the old cleanup JS that was added recently
editor_html = re.sub(r"const urlParams = new URL(window\.location\.href)\.searchParams;.*?\}\);\s*\}\n*", "", editor_html, flags=re.DOTALL)
...
# ---------------------------------------------------------
detail_html = html
detail_html = re.sub(r"window\.addEventListener\('DOMContentLoaded', \(\) => \{\n\s*const urlParams = new URLSearchParams.*?\}\);\n\s*\}\n", "", detail_html, flags=re.DOTALL)
# some previous scripts added logic inside DOMContentLoaded or simply flat script. 
# Let's remove the generic inject block I created earlier:

detail_html = re.sub(r"const urlParams.*?\}\n\s*\}\n*", "", detail_html, flags=re.DOTALL)

# Just to be safe, I'll strip all references to "? Create New" and "서버에서 자동 생성됩니다" from the JS in detail view
