...
def get_category_and_entity(filename):
    ...
    if filename.startswith('content'): return 'Contents & Product', '🎬 Content', 'content-list.html'
    if filename.startswith('version-'): return 'Contents & Product', '🎬 Content', 'content-list.html'
...
for file in files:
    ...
    # 1. Strip out older breadcrumb snippets to avoid duplicates
    html = re.sub(r'<div class="breadcrumb".*?</div>', '', html, flags=re.DOTALL)
    
    # Remove old manual back buttons (like "← Back to Content", "← 버전 히스토리로 돌아가기")
...
print("Breadcrumbs ADDED successfully.")
